...
/*HHT*/
#include "dobot/SetHHTTrigMode.h"
#include "dobot/GetHHTTrigMode.h"
#include "dobot/SetHHTTrigOutputEnabled.h"
#include "dobot/GetHHTTrigOutputEnabled.h"
#include "dobot/GetHHTTrigOutput.h"

bool SetHHTTrigModeService(dobot::SetHHTTrigMode::Request &req, dobot::SetHHTTrigMode::Response &res){
	HHTTrigMode HHT;
	if(req.tagHHTTrigMode==0)HHT=TriggeredOnKeyReleased;
	else if(req.tagHHTTrigMode==1)HHT=TriggeredOnPeriodicInterval;
	res.result = SetHHTTrigMode(HHT);
	return true;
}

bool GetHHTTrigModeService(dobot::GetHHTTrigMode::Request &req, dobot::GetHHTTrigMode::Response &res){
	HHTTrigMode HHT;
	res.result=GetHHTTrigMode(&HHT);
	if(res.result==DobotCommunicate_NoError){
		res.tagHHTTrigMode=HHT;
	}
	return true;
}
bool SetHHTTrigOutputEnabledService(dobot::SetHHTTrigOutputEnabled::Request &req, dobot::SetHHTTrigOutputEnabled::Response &res){
	bool isEnabled=req.isEnabled;
	res.result = SetHHTTrigOutputEnabled(&isEnabled);
	return true;
}
bool GetHHTTrigOutputEnabledService(dobot::GetHHTTrigOutputEnabled::Request &req, dobot::GetHHTTrigOutputEnabled::Response &res){
	bool isEnabled;
	res.result = GetHHTTrigOutputEnabled(&isEnabled);
	if(res.result==DobotCommunicate_NoError){
		res.isEnabled=isEnabled;
	}
	return true;
}
bool GetHHTTrigOutputService(dobot::GetHHTTrigOutput::Request &req, dobot::GetHHTTrigOutput::Response &res){
	bool isTriggered;
	res.result = GetHHTTrigOutput(&isTriggered);
	if(res.result==DobotCommunicate_NoError){
		res.isTriggered=isTriggered;
	}
	return true;
}
void InitHHTServices(ros::NodeHandle &n, std::vector<ros::ServiceServer> &serverVec)
{
    ros::ServiceServer server;
    server = n.advertiseService("/DobotServer/SetHHTTrigMode", SetHHTTrigModeService);
    serverVec.push_back(server);
    server = n.advertiseService("/DobotServer/GetHHTTrigMode", GetHHTTrigModeService);
    serverVec.push_back(server);
    server = n.advertiseService("/DobotServer/SetHHTTrigOutputEnabled", SetHHTTrigOutputEnabledService);
    serverVec.push_back(server);
    server = n.advertiseService("/DobotServer/GetHHTTrigOutputEnabled", GetHHTTrigOutputEnabledService);
    serverVec.push_back(server);
    server = n.advertiseService("/DobotServer/GetHHTTrigOutput", GetHHTTrigOutputService);
    serverVec.push_back(server);
}
/*
 * End effector
